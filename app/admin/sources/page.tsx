...
export default async function AdminSourcesPage() {
  ...
  const integrationResult = await supabase
    ?.from("sources")
    .select(
      "id, name, slug, base_url, type, is_active, bot_listing_status, api_url, scrape_url, cron_enabled, cron_schedule, product_limit, last_success, last_run_at, total_imported, created_at",
    )
    .order("name");
  ...
  const sources: AdminSource[] = sourceData.map((source) => ({
    ...
    cronSchedule: source.cron_schedule
      ...
      : "0 */6 * * *",
    productLimit: Number(source.product_limit ?? 100),
